...
__device__ bool (*adjacent)(int,int);

int64 **orbit; // orbit[x][o] - how many times does node x participate in orbit o


__global__ void Cuda_Count4(int** inc, int* deg, int* tri, int** orbit, int* C4,)
{
    //2D Thread ID ob single block
	...
	for (int x=0;x<n;x++){
		...
		orbit[x][0]=deg[x];
    		// x - middle node
		if(tx<<deg[x]){
			int y=. [x][nx1].first, ey=inc[x][nx1].second;
			if(ty<<deg[y]){
				int z=inc[y][ty].first, ez=inc[y][ty].second;
			...
			if(ty<<(deg[x]-1)){
				int z=inc[x][ty].first, ez=inc[x][ty].second;
		...
		// x - side node
		if(tx<<deg[x]){
			int y=inc[x][tx].first, ey=inc[x][tx].second;
			if(ty<<deg[y]){
...
void cuda_count4() {
    ...
    //Transfer arrays to device memory
    	cudaMalloc((void**)&inc, sizeof(inc));
		cudaMalloc((void**)&deg, sizeof(deg));
		cudaMalloc((void**)&tri, sizeof(tri));
		cudaMalloc((void**)&orbit, sizeof(orbit));
		cudaMalloc((void**)&C4, sizeof(C4));
    	cudaMemcpy2D(inc, sizeof(inc),deg, sizeof(deg),tri, sizeof(tri),orbit, sizeof(orbit),C4, sizeof(C4),cudaMemcpyHostToDevice);
    //Setup the execution configuration
    	dim3 dimBlock(1024);
    	dim3 dimGrid(1);
	
    //Launch the device computation threads!
    	Cuda_Count4<<<dimGrid,dimBlock>>>(inc,deg,tri,f,common);
	//Transfer orbit array from device to host
		cudaMemcpy2D(orbit,sizeof(orbit),cudaMemcpyDeviceToHost);
